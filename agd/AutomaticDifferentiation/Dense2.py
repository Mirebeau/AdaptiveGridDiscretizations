...
class denseAD2(Base.baseAD):
	...
	def __init__(self,value,coef1=None,coef2=None,broadcast_ad=False):
		...
		if ad_generic.is_ad(value):
			raise Base.ADCastError(f"Attempting to cast {type(value)} to incompatible type denseAD2")
		self.value = ad_generic.asarray(value)
		self.coef1 = (np.zeros_like(value,shape=self.shape+(0,)) if coef1 is None 
			else misc._test_or_broadcast_ad(coef1,self.shape,broadcast_ad) )
		self.coef2 = (np.zeros_like(value,shape=self.shape+(0,0)) if coef2 is None 
	...
	def as_func(self,h):
		...
		hh = np.expand_dims(h,axis=0) * np.expand_dims(h,axis=1)
	...
	def __setitem__(self,key,other):
		...
		if self.is_ad(other):
			...
			if osad==0: return self.__setitem__(key,other.value)
				...
				self.coef1=np.zeros_like(self.value,shape=self.coef1.shape[:-1]+(osad,))
				self.coef2=np.zeros_like(self.value,shape=self.coef2.shape[:-2]+(osad,osad))
	...
	def reshape(self,shape,order='C'):
		...
		return self.new(value,self.coef1.reshape(value.shape+(self.size_ad,),order=order),
			self.coef2.reshape(value.shape+(self.size_ad,self.size_ad),order=order))
	...
	def allclose(self,other,*args,**kwargs): 
		return np.allclose(self.value,other.value,*args,**kwargs) and \
		       np.allclose(self.coef, other.coef, *args,**kwargs) and \
		       np.allclose(self.coef2,other.coef2,*args,**kwargs)
	...
	def concatenate(cls,elems,axis=0):
		...
		np.concatenate(tuple(e.coef1 if e.size_ad==size_ad else 
			np.zeros_like(e.coef1,shape=e.shape+(size_ad,)) for e in elems2),axis=axis1),
		np.concatenate(tuple(e.coef2 if e.size_ad==size_ad else 
			np.zeros_like(e.coef2,shape=e.shape+(size_ad,size_ad)) for e in elems2),axis=axis2))
...
def identity(*args,**kwargs):
	...
	return denseAD2(arr.value,arr.coef,
		np.zeros_like(arr.value,shape=arr.shape+(arr.size_ad,arr.size_ad)))
