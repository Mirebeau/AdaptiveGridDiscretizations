...
def ascontiguousarray(a,**kwargs):
	a = asarray(a,**kwargs)
	if isinstance(a,np.ndarray): return np.ascontiguousarray(a)
	elif isinstance(a,_cp_ndarray): return cp.ascontiguousarray(a)
	elif is_ad(a):
		return type(a)(*tuple(ascontiguousarray(e) for e in a.as_tuple()))
	else: raise ValueError(f"ascontiguousarray not applicable to variable of type {type(a)}")
...
def array_members(data,iterables=(tuple,list,dict)):
	...
	Usage : for key,value in ad.Base.array_members(self): print(key,value.nbytes/2**20)
	"""
	arrays = []
	def check(path,arr):
		if isndarray(arr):
...
numpy_implementation = {# Use original numpy implementation
	np.ndim,np.shape,np.moveaxis,np.squeeze,
	np.amin,np.amax,np.argmin,np.argmax,np.max,np.min,
	np.sum,np.prod,
...
def expand_dims(a,axis):
	shape = np.expand_dims(a.value,axis).shape
...
def copy_to(dst,src,*args,**kwargs):
	...
	np.copyto(dst.value,src,*args,**kwargs)
