...
class RecurseRewind:
	...
	def __init__(self,next,initial,params=tuple(),base=5):
		self.next = next
		self.params = params
		self._base = base
	...
	def __call__(self,index=None):
		"""The value of the current iteration if index is None, otherwise moves to index"""
		if index is not None: self.moveto(index)
		return self.value
	...
	def moveto(self,index):
		...
		# Possible improvements here, especially for functions which can be iterated at a reduced 
		# cost, e.g. by merging the final and initial substeps of symplectic schemes for wave PDEs
		if index < self.index: 
			for i in range(self.index-index): self.rewind()
		if index > self.index:
			for i in range(index-self.index): self.advance()
