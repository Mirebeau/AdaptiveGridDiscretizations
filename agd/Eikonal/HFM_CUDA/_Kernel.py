...
def SetKernel(self):
	...
	if self.hasTips:
		...
		# Special backtracking mode for the DubinsState model
		if self.nstates==1: self.geodesic_mixed_state=False
		else: self.geodesic_mixed_state = self.GetValue('geodesic_mixed_state',
			default=self.nstates>1, help="DubinsState backtracking in the propability simplex")
		...
		# If the geodesic does not change pixel within nymin_delay time steps, it is regarded
		# as stationnary. Changes in the mixed_state are not considered.
		nymin_delay = int(self.GetValue('geodesic_Stationnary_delay',
			default=8.*np.sqrt(self.ndim)/geodesic.policy.step 
			#* (self.nstates if self.geodesic_mixed_state else 1)
		...
		geodesic.traits = { # Suggested defaults
			**nofront_traits,
			...
			'mixed_state_macro':self.geodesic_mixed_state
			}
	...
	self.exportFlowState = self.nstates>1 and self.GetValue('exportFlowState',default=False,
		help="Export flow between the states of the DubinsState vehicle")
	flow_weightpos_macro = self.exportActiveNeighs 
	flow_state_macro = self.exportFlowState or (self.hasTips and self.geodesic_mixed_state)
	self.flow_needed = (self.forwardAD or self.reverseAD or self.exportGeodesicFlow 
		or flow_weightpos_macro or flow_state_macro or not online_flow)
	geodesic_outofline = None if online_flow else geodesic

	if self.flow_needed:
		flow.traits = {
			**nofront_traits,
			'flow_weightpos_macro':flow_weightpos_macro,
			'flow_state_macro':flow_state_macro,
		}
	...
	eikonal.args['geom'] = cp.ascontiguousarray(cp.asarray(eikonal.args['geom']))
	if self.precompute_scheme:
